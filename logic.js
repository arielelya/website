function setClickableTooltip(target, content){
  ...
  document.addEventListener('DOMContentLoaded', function () {
    ...
    const calculator = document.getElementById('calcu');

    toggleButton.addEventListener('click', function() {
    if (calculator.style.display === "none") {
        calculator.style.display = "block";
        toggleButton.style.display = "none"; // Hide the "Click Me" button
    } else {
        calculator.style.display = "none";
    }

    });
    const result = document.getElementById('result'); // Get the display element

    document.getElementById('calcu').addEventListener('click', function(e) {
        const target = e.target; // Clicked element

        if (target.matches('input[type="button"]')) { // Check if a button was clicked
            const value = target.value; // Get button value

            if (value === '=') {
                // If equals button was clicked, calculate and show result
                try {
                ...
                // If clear button was clicked, clear display
                result.value = '';
            } else {
                // Otherwise, append button value to display
                result.value += value;
            }
        }
    });
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    form.addEventListener('submit', function(event) {
        ...
        messageText.textContent = 'Form Submitted Successfuly!';
